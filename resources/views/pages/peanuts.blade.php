            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-8">
                ...
                @forelse($products as $product)
                    <div class="bg-white border border-stone-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300 flex flex-col group">
                        ...
                        <!-- Product Thumbnail with Inline Fallback Logic -->
                        <div class="aspect-[4/3] bg-stone-50 flex items-center justify-center relative overflow-hidden border-b border-stone-100">
                            <img src="{{ $product->image ? asset('storage/' . $product->image) : asset('image/pn.jpg') }}" 
                                 alt="{{ $product->name }}" 
                        ...
                        <div class="p-5 flex-grow flex flex-col justify-between">
                            <div>
                                <div class="flex justify-between items-baseline mb-2 gap-2">
                                    <h3 class="font-sans text-lg font-bold text-peanut truncate" title="{{ $product->name }}">
                                        {{ $product->name }}
                                    </h3>
                                    <span class="text-xs font-bold text-golden whitespace-nowrap shrink-0">
                                        NPR {{ number_format($product->price) }}
                                        @if($product->unit)
                                            <span class="text-stone-400 font-medium">/ {{ $product->unit }}</span>
                                        @endif
                                    </span>
                                </div>
                                <p class="text-xs sm:text-sm text-peanut/70 mb-6 leading-relaxed font-light line-clamp-3" title="{{ $product->description }}">
                                    {{ $product->description }}
                                </p>
                            </div>
                            ...
                            <div class="mt-auto grid grid-cols-2 gap-2">
                                ...
                                <!-- WhatsApp Button -->
                                <a href="https://wa.me/YOUR_PHONE_NUMBER?text={{ urlencode('Hello, I am interested in ordering: ' . $product->name) }}" target="_blank" 
                                   class="flex items-center justify-center gap-1.5 w-full bg-[#25D366] hover:bg-[#20bd5a] text-white text-[10px] sm:text-xs font-bold uppercase tracking-wider py-3 rounded-xl transition shadow-sm" title="Order via WhatsApp">
                                ...
                                <a href="https://m.me/your-facebook-page" target="_blank" 
                                   class="flex items-center justify-center gap-1.5 w-full bg-[#0084FF] hover:bg-[#0073e6] text-white text-[10px] sm:text-xs font-bold uppercase tracking-wider py-3 rounded-xl transition shadow-sm" title="Order via Messenger">
                                    <svg class="w-4 h-4 fill-current shrink-0" viewBox="0 0 24 24">
                                        <path d="M12 0C5.373 0 0 4.974 0 11.111c0 3.498 1.744 6.614 4.469 8.562V24l4.093-2.271A12.33 12.33 0 0012 22.222c6.627 0 12-4.974 12-11.111S18.627 0 12 0zm1.293 14.246l-3.08-3.297-6.015 3.297 6.615-7.022 3.138 3.297 5.957-3.297-6.615 7.022z"/>
                                    </svg>
                                    Messenger
                                </a>
                ...
                @empty
                    <!-- Empty State -->
                    <div class="col-span-1 md:col-span-2 lg:col-span-3 text-center py-16 bg-stone-50 border border-dashed border-stone-200 rounded-2xl">
                        <span class="text-4xl block mb-3 opacity-50">🥜</span>
                        <h3 class="text-lg font-bold text-peanut">Harvest in Progress</h3>
                        <p class="text-sm text-stone-500 mt-1">Our catalog is currently empty. Please check back soon!</p>
                    </div>
                @endforelse
