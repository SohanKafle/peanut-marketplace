<x-app-layout>
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
        <div>
            ...
            <p class="text-xs sm:text-sm text-stone-500 mt-1">Manage your home inventory, occupancy limits, and pricing
                details.</p>
        </div>
    ...
    <!-- Search & Filter Form -->
    <form id="search-form" action="{{ route('admin.homestays.index') }}" method="GET"
        ...
        <div class="relative flex-1">
            ...
            <input type="text" name="search" id="search-input" value="{{ request('search') }}"
                placeholder="Type to search properties..." oninput="autoSubmit()"
            ...
            <div
                ...
                <div class="w-full md:w-32 h-20 md:h-20 bg-stone-100 rounded-xl overflow-hidden flex-shrink-0 relative">
                    @if ($homestay->image_path)
                        <img src="{{ asset('storage/' . $homestay->image_path) }}" class="w-full h-full object-cover"
                            alt="Property">
                    @else
                ...
                <div class="flex-1 min-w-0 w-full space-y-1">
                    <div class="flex items-center gap-2">
                        <span
                            class="text-[10px] text-stone-400 font-bold uppercase tracking-wider">{{ $homestay->location }}</span>
                    </div>
                    <h3 class="font-bold text-stone-900 text-base truncate pr-4">
                        {{ $homestay->name }}
                    </h3>
                    <p class="text-xs text-stone-500 truncate">Hosted by <span
                            class="font-medium text-stone-700">{{ $homestay->host_name }}</span> •
                        {{ $homestay->capacity }} Guests</p>
                </div>
                ...
                <div
                    ...
                    <form action="{{ route('admin.homestays.destroy', $homestay->id) }}" method="POST"
                        onsubmit="return confirm('Remove property listing?');" class="flex-1 md:flex-none">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
