<x-app-layout>
    <div class="min-h-screen bg-stone-50 py-8">
        <div class="max-w-5xl mx-auto px-4">
            ...
            <!-- Header -->
            <div class="mb-8">
                <h1 class="text-3xl font-sans font-bold text-stone-900">Edit Hero Slide</h1>
                <p class="text-stone-500 mt-1">Update slide title, active status, or replace the banner image.</p>
            </div>

            <form id="hero-slide-form" action="{{ route('admin.hero-slides.update', $slide->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="bg-white border border-stone-200 rounded-2xl p-6 shadow-sm">
                            ...
                            <input type="text" name="title" value="{{ old('title', $slide->title) }}" placeholder="e.g. Summer Special Banner" required
                        ...
                        <div class="bg-white border border-stone-200 rounded-2xl p-6 shadow-sm">
                            ...
                            <div class="relative aspect-video rounded-xl bg-stone-50 border-2 border-dashed border-stone-300 flex items-center justify-center cursor-pointer overflow-hidden group hover:border-peanut transition">
                                <img id="image-preview" 
                                     ...
                                     class="absolute inset-0 w-full h-full object-cover {{ $slide->image_path ? '' : 'hidden' }}">
                                
                                <input type="file" name="image" class="absolute inset-0 opacity-0 cursor-pointer" onchange="preview(event)">
                                
                                <div id="upload-placeholder" class="flex flex-col items-center justify-center text-center p-4 {{ $slide->image_path ? 'hidden' : '' }}">
                                    ...
                                    <span class="text-[10px] font-bold text-stone-400 uppercase group-hover:text-peanut">Click to upload new photo</span>
                                </div>
                            </div>
                            <p class="text-[11px] text-stone-400 mt-2">Leave blank if you want to keep the current image.</p>
                        </div>
                    ...
                    <div class="space-y-6">
                        <div class="bg-white border border-stone-200 rounded-2xl p-6 shadow-sm">
                            ...
                            <select name="is_active"
                                ...
                                <option value="1" {{ old('is_active', $slide->is_active ? '1' : '0') == '1' ? 'selected' : '' }}>Active</option>
                                <option value="0" {{ old('is_active', $slide->is_active ? '1' : '0') == '0' ? 'selected' : '' }}>Inactive</option>
                            </select>
                        </div>
                    </div>
                </div>
