<x-app-layout>
    ...
    @endif
                ...
                <!-- Action Zone: Clearly visible and easy to tap -->
<div class="flex items-center gap-2 w-full md:w-auto px-2 mt-4 md:mt-0">
    
    <!-- Edit Button -->
    <a href="{{ route('admin.stories.edit', $story->id) }}" 
       class="flex-1 md:flex-none flex items-center justify-center gap-1.5 px-4 py-2.5 bg-stone-100 hover:bg-stone-200 text-stone-700 rounded-lg text-xs font-bold uppercase transition">
    ...
    <form action="{{ route('admin.stories.destroy', $story->id) }}" method="POST" class="flex-1 md:flex-none">
        ...
        <button type="submit" onclick="return confirm('Are you sure you want to delete this story?')"
                class="w-full md:w-auto flex items-center justify-center gap-1.5 px-4 py-2.5 bg-rose-50 hover:bg-rose-100 text-rose-600 rounded-lg text-xs font-bold uppercase transition">
