<x-app-layout>
    <div class="min-h-screen bg-stone-50 py-8">
        <div class="max-w-7xl mx-auto px-4">
            ...
            <form method="GET" id="filter-form" action="{{ route('admin.products.index') }}" class="flex flex-col md:flex-row gap-3 mb-6 bg-stone-100/60 p-3 rounded-2xl border border-stone-200/60">
                <div class="relative flex-1">
                    ...
                    <input type="text" name="search" id="search-input" value="{{ request('search') }}" placeholder="Search products..." autocomplete="off"
                ...
                @forelse($products as $product)
                    <div class="bg-white border border-stone-200 rounded-2xl p-4 flex flex-col lg:flex-row lg:items-center justify-between gap-4 hover:shadow-sm hover:border-stone-300 transition-all duration-200">
                        ...
                        <div class="flex items-center justify-end gap-2 border-t lg:border-t-0 pt-3 lg:pt-0 border-stone-100 w-full lg:w-auto">
                            ...
                            <form action="{{ route('admin.products.destroy', $product->id) }}" method="POST" onsubmit="return confirm('Remove listing?');" class="flex-1 lg:flex-none">
                                @csrf
                                @method('DELETE')
