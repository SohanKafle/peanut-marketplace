...
<body class="bg-cream font-sans text-peanut antialiased flex flex-col min-h-screen selection:bg-peanut selection:text-white">
    ...
    <header class="bg-white/95 backdrop-blur-md sticky top-0 z-50 border-b border-cream-dark transition-all">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 md:h-20 flex items-center justify-between">
            ...
            <nav class="hidden lg:flex items-center gap-8 font-medium text-xs tracking-wider uppercase">
                ...
                <a href="/about" @class([
                    ...
                    'text-peanut/70 border-transparent hover:text-peanut hover:border-golden' => !request()->is('about*')
                ])>About</a>
                
                <a href="/peanuts" @class([
                    'transition-all py-1 border-b-2',
                    'text-peanut border-golden font-bold' => request()->is('peanuts*'),
                    'text-peanut/70 border-transparent hover:text-peanut hover:border-golden' => !request()->is('peanuts*')
                ])>Peanuts</a>
                
                <a href="/homestays" @class([
                    'transition-all py-1 border-b-2',
                    'text-peanut border-golden font-bold' => request()->is('homestays*'),
                    'text-peanut/70 border-transparent hover:text-peanut hover:border-golden' => !request()->is('homestays*')
                ])>Homestays</a>
                
                <a href="/stories" @class([
                    'transition-all py-1 border-b-2',
                    'text-peanut border-golden font-bold' => request()->is('stories*'),
                    'text-peanut/70 border-transparent hover:text-peanut hover:border-golden' => !request()->is('stories*')
                ])>Stories</a>
            </nav>

            <div class="hidden lg:block">
                <a href="/connect" class="inline-flex items-center justify-center bg-peanut text-white text-xs font-bold uppercase tracking-wider px-5 py-3 rounded-xl shadow-sm hover:bg-golden hover:text-peanut transition-all duration-300">
                    Connect & Support
                </a>
            </div>

            <button id="mobile-menu-btn" class="lg:hidden p-2 text-peanut focus:outline-none rounded-lg hover:bg-cream transition-colors" aria-label="Toggle Menu">
        ...
        <div id="mobile-menu" class="hidden lg:hidden bg-white border-t border-cream-dark shadow-xl absolute w-full left-0 z-50 transition-all duration-300">
            <div class="px-4 pt-3 pb-6 space-y-1.5">
                ...
                <div class="pt-4 mt-2 border-t border-cream-dark">
                    <a href="/connect" class="w-full text-center block bg-peanut text-white font-bold uppercase tracking-wider py-3.5 rounded-xl text-xs hover:bg-golden hover:text-peanut transition-all duration-300 shadow-sm">
                        Connect & Support
                    </a>
                </div>
            </div>
