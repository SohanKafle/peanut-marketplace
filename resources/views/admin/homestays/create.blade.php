<x-app-layout>
    <div class="min-h-screen bg-stone-50 py-8">
        <div class="max-w-7xl mx-auto px-4">
            <div class="mb-8">
                <h1 class="text-3xl font-sans font-bold text-stone-900">
                    {{ request()->routeIs('*edit*') ? 'Edit Property' : 'Add New Property' }}
                </h1>
                <p class="text-stone-500">Fill in the details to list a homestay.</p>
            </div>

            <form action="{{ request()->routeIs('*edit*') ? route('admin.homestays.update', $homestay) : route('admin.homestays.store') }}" 
                  method="POST" 
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div class="bg-white p-6 md:p-8 rounded-3xl border border-stone-200 shadow-sm">
                            <div class="space-y-6">
                                <div>
                                    ...
                                    <input type="text" name="name" value="{{ old('name', $homestay->name ?? '') }}" required
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    <div>
                                        ...
                                        <input type="text" name="host_name" value="{{ old('host_name', $homestay->host_name ?? '') }}" required
                                            class="w-full px-4 py-3 bg-white border border-stone-300 rounded-xl focus:border-peanut outline-none text-stone-800">
                                    </div>
                                    <div>
                                        <label class="block text-[10px] font-black text-stone-400 uppercase tracking-widest mb-1.5">Location</label>
                                        <input type="text" name="location" value="{{ old('location', $homestay->location ?? '') }}" required
                                            class="w-full px-4 py-3 bg-white border border-stone-300 rounded-xl focus:border-peanut outline-none text-stone-800">
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                                    <div>
                                        ...
                                        <input type="number" name="capacity" value="{{ old('capacity', $homestay->capacity ?? '') }}" required
                                            class="w-full px-4 py-3 bg-white border border-stone-300 rounded-xl focus:border-peanut outline-none text-stone-800">
                                    </div>
                                    <div>
                                        <label class="block text-[10px] font-black text-stone-400 uppercase tracking-widest mb-1.5">Price (NPR/Night)</label>
                                        <input type="number" name="price_per_night" value="{{ old('price_per_night', $homestay->price_per_night ?? '') }}" required
                                            class="w-full px-4 py-3 bg-white border border-stone-300 rounded-xl focus:border-peanut outline-none text-stone-800">
                                    </div>
                                </div>
                                ...
                                <div>
                                    ...
                                    <textarea name="description" rows="5"
                                        class="w-full px-4 py-3 bg-white border border-stone-300 rounded-xl focus:border-peanut outline-none text-stone-800">{{ old('description', $homestay->description ?? '') }}</textarea>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="lg:col-span-1 space-y-6">
                        <div class="bg-white p-6 md:p-8 rounded-3xl border border-stone-200 shadow-sm space-y-5">
                            ...
                            <!-- WhatsApp Field -->
                            <div>
                                <label class="block text-[10px] font-black text-stone-400 uppercase tracking-widest mb-1.5">
                                    WhatsApp / Contact Number
                                </label>
                                <input type="text" 
                                       name="whatsapp_number" 
                                       placeholder="e.g. 977980000000"
                                       value="{{ old('whatsapp_number', $homestay->whatsapp_number ?? '') }}"
                                       class="w-full px-4 py-3 bg-white border border-stone-300 rounded-xl focus:border-peanut outline-none text-stone-800">
                                <span class="text-[11px] text-stone-400 mt-1 block">Include country code without + (e.g. 977980000000)</span>
                            </div>
